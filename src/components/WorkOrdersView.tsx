...
export function WorkOrdersView({
  ...
  const handleQuickWhatsApp = (e: React.MouseEvent, order: WorkOrder) => {
    e.stopPropagation();
    try {
      sendOrderWhatsApp(order);
      showSuccess('WhatsApp Abierto', `Se generó el mensaje de la OT ${order.numeroOrden}.`);
    } catch (err) {
      console.error(err);
      showError('Error de WhatsApp', 'No se pudo abrir WhatsApp.');
    }
  };

  const handleQuickPDF = (e: React.MouseEvent, order: WorkOrder) => {
    e.stopPropagation();
    try {
      downloadOrderPDF(order);
      showSuccess('PDF Descargado', `Se generó el comprobante de la OT ${order.numeroOrden}.`);
    } catch (err) {
      console.error(err);
      showError('Error de PDF', 'Ocurrió un error al generar el PDF.');
    }
  };
                      ...
                      >
                        ...
                        <div className="pt-2 border-t border-slate-100 flex items-center justify-between gap-1.5">
                          <div className="flex items-center gap-1">
                            ...
                            <button
                              type="button"
                              onClick={(e) => handleQuickPDF(e, order)}
                      ...
                      <td className="p-3.5 text-center">
                        <div className="flex items-center justify-center gap-1">
                          ...
                          <button
                            ...
                            className="px-2 py-1 bg-amber-500/10 text-amber-800 hover:bg-amber-500 hover:text-slate-950 font-bold rounded-lg transition-colors text-[11px] flex items-center gap-1 cursor-pointer"
                            title="Editar órden"
                          >
